  ...
  try {
    ...
    switch (tool) {
      ...
      case "create_followup_task": {
        ...
        const raw_text = params.raw_text ? String(params.raw_text) : null;
        const root_cause = params.root_cause ? String(params.root_cause) : null;
        const resolution = params.resolution ? String(params.resolution) : null;
        const evidence_urls = Array.isArray(params.evidence_urls)
          ? params.evidence_urls.map((u: unknown) => String(u)).filter(Boolean)
          : [];

        // Resolve asset_code -> asset_id (find by org_id + code, else create),
        // mirroring the manual UI logic in AddFieldRecordDialog.tsx.
        let asset_id: string | null = null;
        const asset_code = params.asset_code ? String(params.asset_code).trim() : "";
        if (asset_code) {
          ...
          if (asset?.id) {
            ...
            const { data: createdAsset } = await admin
              .from("assets")
              .insert({ org_id: orgId, code: asset_code, name: asset_code })
              .select("id")
              .single();
        ...
        const { data, error } = await admin
          ...
          .insert({
            ...
            asset_id,
            evidence_urls,
            // quality_score is computed server-side by a BEFORE INSERT trigger.
          })
          .select()
          .single();
